  ...
  >
    <div class="">
      ...
      <ul class="list" v-if="houseList && houseList.length" ref="list">
        <li v-for="item in houseList" :key="`${item.uid}-${item.source}`">
          <div class="left">
          ...
          <div class="right">
            <div class="content">
              <!--<a class="title" :href="`/#/detail/${item.uid}`" target="_blank">{{item.title ?-->
                <!--item.title : item.location}}</a>-->
              <router-link :to="`/detail/${item.uid}`" tag="a" class="title">{{item.title ?
