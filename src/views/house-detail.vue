<template>
  <div class="detail" :class="{'is-mobile':isMobile}">
    <header>
      <router-link tag="div" class="name" to="/">小猪租搜房</router-link>
      <div class="form" v-if="detail">
        <a class="title" :href="detail.online_url" target="_blank">{{detail.title}} <font color=red>&lt;&lt;&lt; 点击跳转到原文</font></a>
    ...
    <div class="content" v-if="detail">
      <div class="images-wrap" v-if="detail.pictures && detail.pictures.length">
        <el-carousel indicator-position="inside" :interval="0" :autoplay="false" arrow="always">
          <el-carousel-item v-for="item in JSON.parse(detail.pictures)" :key="item">
    ...
    methods: {
      ...
      async getDetail() {
        return new Promise(async (resolve,reject)=>{
          try {
            const data = await this.$v2.get(`/houses/${this.uid}?tdsourcetag=s_pcqq_aiomsg`);
