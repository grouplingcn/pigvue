  ...
  <div class="map" :class="{'showInfo':isMobile}">
    <header v-if="!isMobile">
      <div class="title" v-if="!isMobile">
        <router-link to="/">小猪租搜房</router-link>
        <el-dropdown>
      ...
      <ul>
        <li>
          ...
          <div class="displaySource">
            <span @click="selectSource('')">全部</span>
      ...
      <div class="container-wrap">
        ...
        <template v-if="!isMobile">
          <div class="card">
            <h4>出行到达圈查询</h4>
            <div class="card-item">
              <span class="card-name">租房地点</span>
              <el-input
                  id="keyword"
                  class="card-value"
                  size="mini"
          ...
          <span class="highlight-text">特此声明:房源信息来自网络爬虫。在下方输入租房地点点击查询开启找房之旅。 
            <!-- <a href="https://wj.qq.com/s/2953926/aabe" target="_blank" class="do-more-better">帮我们做得更好?</a> -->
          </span>
          <!-- <div class="icon-tips"> -->
            <!-- <ul> -->
              <!--<li class="btn">-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refresh(lnglat = {}) {
        ...
        if (lnglat.lat && lnglat.lng) {
          coordinate.lat = lnglat.lat
          coordinate.lng = lnglat.lng
        }
        let query = this.$route.query;
        let params = Object.assign({}, query, this.transformParams(), coordinate);
        this.$router.push({query: params});

        setTimeout(() => {
          console.log('refresh')
          this.init(this.location,lnglat);
        }, 200);
      },
      cityLocation(item) {
        this.location = item;
      ...
      async preview(item) {
        ...
        try {
          ...
          await asyncComponent(com, {
            props: {
              images: JSON.parse(item.pictures),
              isMobile: self.isMobile
            }
          }, (template) => {
            this.view = template;
      ...
      search(type,initMapStatus = false) {
        let self = this;
        self.searching = true;
        ...
        self.arrivalRange.search(self.lnglat, self.times, function (status, result) {
          ...
          if (result.bounds) {
            ...
            //TODO 优化:通过高德地图的地址来筛选附近的数据信息
            // self.mapHouseList = [];
            // self.houseList = [];
            
            if(initMapStatus === false) {
              self.refresh(self.lnglat);
            }
            // alert(self.searching)
      ...
      houseListClick(item) {
        if (item.geocodes && item.geocodes.location) {
          ...
          this.map.setZoomAndCenter(16, [item.geocodes.location.lng,item.geocodes.location.lat]);
      ...
      async getList() {
        ...
        let data = this.$v2.post('/houses', params);
            ...
            return new Promise(async (resolve, reject) => {
              ...
              try {
                ...
                if (status === "complete" && result.info && result.info.toLocaleLowerCase() === 'ok') {
                  ...
                  let telephoneNumber = item.telephone  ? "电话：" + item.telephone : "电话：前往获取";
                  let sourceContent = item.source ? " 来源：" + item.display_source + " 发布于 " + item.pub_time.slice(0,10) : "";
        ...
        return new Promise((resolve, reject) => {
          code.getLocation(name, (status, result) => { // 城市中心点
            if (status === "complete" && result.info === 'OK') {
              ...
              positionPicker.start(location);//标记点
      ...
      async init(location,lnglat) {
        ...
        try {
          ...
          this.code = code;
          if (JSON.stringify(lnglat) !== '{}'){
            positionPicker.start(lnglat);//标记点
          }else{
    ...
    async mounted() {
      ...
      let key = `23b79ff0622b90e22e441156f65dee4e`;
      ...
      this.sockets.subscribe('new_data', (data) => {
        ...
        for (var key in arrayData) {
          ...
          for (var k in this.mapHouseList) {
            if(this.mapHouseList[k]['uid'] == arrayData[key]['uid'] && this.mapHouseList[k]['source'] == arrayData[key]['source']) {
              arrayData.splice(key,1)
            }
          }
        }
        if(arrayData.length > 0) {
          try{
            this.addMaker(this.map, arrayData, code, this.mapObj);
          } catch (e) {
    ...
    sockets: {
      connect() {
        console.log('socket connect success at map part')
      },
    }
  }
</script>
